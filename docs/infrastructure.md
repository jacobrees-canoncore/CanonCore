...
| Root Directory | `apps/web`, with *include files outside the root directory* on |
| Framework Preset | Next.js |
| Function region | `lhr1` (London) |
| Preview protection | Off. Preview URLs are public. |

The root directory and the framework preset were set by CAN-22 on 10 August 2026, when
`apps/web` arrived; before that the project built the repository root and served the holding page
described below. Root Directory is Vercel's documented mechanism for a monorepo — a project points
at "the directory within your monorepo that you want to deploy"
([Using monorepos](https://vercel.com/docs/monorepos)) — and *include files outside the root
directory* is what makes the workspace root, and therefore `packages/config`, visible to the build.
Neither is expressed in this repository: they are project settings, so a file here cannot assert
them and this table is the only record.

**The build honours `packageManager`, and runs a pnpm newer than the documented set.** Vercel's
[Package Managers](https://vercel.com/docs/package-managers) table lists pnpm "6, 7, 8, 9, 10", and
says the version is taken from the lockfile unless Corepack is enabled. Neither held on 10 August
2026. The first `apps/web` build logged:

```
Detected `pnpm-lock.yaml` version 9 generated by pnpm@10.x with package.json#packageManager pnpm@11.20.0
…
Done in 12.7s using pnpm v11.20.0
```

So pnpm 11 works, the root `packageManager` field is what selects it, and no
`ENABLE_EXPERIMENTAL_COREPACK` was set. Read this as an observation with a date on it rather than a
guarantee: the documentation says otherwise, so a future build image could match the docs instead.
If one ever does, the symptom is an install failure and the fix is to pin `packageManager` to a
version in that table.
...
## What serves the production domain

`apps/web`, the Next.js application, since CAN-22 landed on 10 August 2026. The page a stranger
sees is `apps/web/app/page.tsx`.

It replaced a holding page — a single `public/index.html` with a root `vercel.json` setting
`outputDirectory` to `public` — which CAN-18 committed to the repository so that a production build
of a repository with no application could not produce a 404. **Any** push to `main` triggers a
production build, so a documentation-only merge would otherwise have taken the site down. CAN-22
deleted both files in the same change that added the application, which is the only ordering that
leaves no interval with neither.

The page carries `robots: { index: false, follow: false }`, as the holding page did. There is
nothing to index until there is a product and a terms of service behind it (CAN-21).
