...
| Security-settings roster vs the repository | **Locally only** | `security_and_analysis` comes back only to a caller with admin on the repository, and `permissions:` accepts no scope that grants one — the same wall as the secret roster. It is also what tells the other two calls' `404` apart from a refusal, so a runner reads none of the seven rather than some of them. [`../infrastructure.md`](../infrastructure.md) → *Dependency and secret scanning* has the three calls |
