...
| The Provider baseline's composed context | CI and locally | Both halves are files here, so it can never skip. What it does not reach is the Provider rulesets requiring the composed string — `provider-tmdb`'s has existed since 21 August 2026 — because a check here would need a roster of Provider repositories to iterate, and `scripts/provision-provider-repository.ts` already reads those back on a re-run. **That re-run is not drift detection**: it writes before it reads and nothing schedules it, which is **CAN-145 Give the Provider provisioning a report-only mode, and something that runs it**. [`../infrastructure.md`](../infrastructure.md) → *What the first real run showed* |
...
| The glossary's `_Avoid_` lists vs every tracked document | CI and locally | Local files throughout. It walks the same `git ls-files` listing as the two rows above, so it skips exactly where they do and fails rather than passes on an empty one. The vocabulary is read out of [`../../CONTEXT.md`](../../CONTEXT.md) rather than written into the script, and so is every exception. **CAN-129 Enforce the glossary's _Avoid_ lists with a check, instead of a reviewer's attention** built it, because [`CODING_STANDARDS.md`](../../CODING_STANDARDS.md) → *Domain language* made an `_Avoid_` word used for the concept it is listed against a finding and nothing but a reviewer's attention was enforcing it. What decides that a word is the banned sense, and what the rules deliberately do not reach, is written beside them in [`../../scripts/lib/doc-checks.ts`](../../scripts/lib/doc-checks.ts); an exception lives in [`../../CONTEXT.md`](../../CONTEXT.md) → *Language* and nowhere else. `docs/research/` is out of scope, which that file's *Using these documents* settles: it is not domain documentation, and one of its documents quotes the very violations this landed with |
