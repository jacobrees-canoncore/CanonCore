...
| Revised | 20 August 2026 — **the first Story page ships, and adds its own case to the non-linkification control.** Carried by [CAN-25 The catalogue: Version, part of, Anchor, canonical version](https://linear.app/jacobrees-canoncore/issue/CAN-25). **No level, finding or answer changed, and no `s.9(4)` redo is owed.** Two things moved and neither is a trigger: the surface Step 4 expected from CAN-27 arrived earlier and carries its case in `no-linkification.test.tsx`, and the closed list of permissible anchors gained `/` — which Step 4 says in terms is not the change to watch for. **Every `href` this service renders is still a literal in the repository**, which is the clause the finding rests on: the Story page names what a Story is part of rather than linking to it, and nothing links to the page itself |
| Revised | 21 August 2026 — **a seventh surface ships, and it is the first that renders nothing from outside.** Carried by [CAN-60 Gate the front end on bytes, budgets and React lint](https://linear.app/jacobrees-canoncore/issue/CAN-60), which adds `/privacy/analytics`, the objection route [ADR-0020](../adr/0020-no-cookie-consent-banner.md) requires before analytics may run. **No level, finding or answer changed, and no `s.9(4)` redo is owed.** Two things moved and Step 4 names neither as a trigger: the closed list of permissible anchors gained `/privacy/analytics`, a literal, and that page's own case joined `no-linkification.test.tsx`. **Every string the page renders is prose written in this repository**, and the one anchor on it is `/` — so it neither introduces an ingress nor touches the clause finding 2c rests on. **The terms of service were amended in the same change**, adding a third data disclosure beside Resend's and Sentry's: Vercel counts visits, no cookie is set, the address is reduced before it is sent, the counts are never joined to an account, and the objection route is named. That is the shape [CAN-81 Disclose Sentry's US error storage in the terms of service](https://linear.app/jacobrees-canoncore/issue/CAN-81) set on 16 August 2026, and it changes no level or finding either. The terms are still unpublished |
| Revised | 21 August 2026 — **the capability declaration is read, stored and refused on, and a fifteenth surface ships with it.** Carried by [CAN-104 Read a Provider's capability declaration, and refuse what it does not serve](https://linear.app/jacobrees-canoncore/issue/CAN-104). **No level, finding or answer changed, and no `s.9(4)` redo is owed** — the reasoning is in Step 4, under *Why reading a capability declaration triggers no redo*. In summary: it adds no ingress a user can reach, it is the first surface to render a Provider's prose and it renders every word of it as text, and what it builds is the refusal two of these records already rely on |
...
| **Nothing any other party wrote is linkified** | CSAM URLs, terrorism, drugs, weapons, fraud | Answers risk factor 7b — with the caveat under *Finding or encountering content, re-derived* about what answering *No* does not remove | **Partly, and the assertion strengthened on 17 August 2026.** The *prohibition* is origin-blind over every `.ts` and `.tsx` file under `apps/web/src`: [`apps/web/eslint.config.mjs`](../../apps/web/eslint.config.mjs) carries `react/no-danger`, which is a real guard because JSX escapes what it interpolates, and a restricted-import group naming the markdown renderers and autolinkers most likely to be reached for. **That group matches package names, so it is a tripwire and not a proof** — a renderer it does not name gets through, and an `<a href>` written by hand gets past both rules. The *rendered assertion* is [`no-linkification.test.tsx`](../../apps/web/src/app/no-linkification.test.tsx), which is the only check that sees any of that. **It no longer counts anchors; it pins the exact set of them**, because CAN-24 A signed-in and a signed-out path gave the product its first navigation and an assertion of *zero* anchors would have been failed by a link to `/sign-in`. Every rendered `href` must now be one of a closed list of this application's own routes, so a data-derived `href` fails whether or not it looks like a link — which is a stronger property than the count it replaced, and the one the finding actually needs. It covers the surfaces drawing text today: the front page signed in and signed out, and the four account forms. **The Story page** joined them on 20 August 2026, and is the one whose every line is a value from outside; it is also the first surface to link anywhere but an account page, and what it links to is `/`, a literal — the Story it is part of is named rather than linked to, and nothing links to the page itself. **The counting-visits page** joined on 21 August 2026 and is the mirror of it: every string on it is prose written here. It is covered anyway, because what the finding rests on is the exact set of anchors over *every* surface rather than over the risky ones. **The Sources page** joined on 21 August 2026 and is the first surface on which a Provider's own prose is rendered, ahead of both tickets below: it draws a Source's name, its licence, the notices it prescribes and the conditions on them, all of it text and none of it an anchor, and **it is the first surface where four of the strings are URLs the Provider chose** — the Source's address, the licence's, the credit's link and the logo's. The logo is named rather than fetched, which is a further step than the prohibition asks for and is recorded because it is a different act from rendering a string. CAN-27 Orderings and Placements, and the imported broadcast Ordering (the Ordering page), CAN-26 Import a series from TMDB, with the overlay behind it (a Listed Provider's prose on a *record*) and CAN-113 Add a Provider by pasting its URL (a pasted one's) each add their own |
...
- **Linkifying anything this service did not author** — would switch on the hyperlinking risk factor
  ...
  [CAN-25 The catalogue: Version, part of, Anchor, canonical version](https://linear.app/jacobrees-canoncore/issue/CAN-25)
  the Story page, which landed on 20 August 2026 with its own case and with `/` added to the closed
  list — neither of which is this trigger;
  [CAN-27 Orderings and Placements, and the imported broadcast Ordering](https://linear.app/jacobrees-canoncore/issue/CAN-27)
  the Ordering page, which is where the prohibition meets a URL typed into an Argument;
...
> **Why reading a capability declaration triggers no redo, though it renders a Provider's prose.**
...
> makes the application read what a Provider declares about its Source, store it, refuse what the
> declaration does not cover, and show the whole of that on `/sources`. **Both lists were read against
> it before it shipped**, as they were for the changes below. Three entries could have been engaged
> and none is:
>
> - **"Rendering a Provider's prose anywhere the controls above do not reach"**, which is one of the
>   four moves listed under the pasted-Provider entry. The controls *do* reach it, and which ones is
>   the whole of the answer. Non-linkification: the page's own case is in
>   [`no-linkification.test.tsx`](../../apps/web/src/app/no-linkification.test.tsx), every value it
>   draws is text, and the four URLs a declaration carries are named rather than made followable —
>   the logo included, which is named rather than fetched. A corpus small enough to review: a row is
>   one Provider, put there by the operator. Reporting and takedown: the reporting address is in the
>   footer of that page like every other, and the takedown for such a row is the purge that already
>   exists ([`runbook.md`](../runbook.md) → *A Source's licence terminates*), which removes the row
>   and everything that Source ever said.
> - **The linkification bullet.** `/sources` joins the closed list of permissible anchors, and that
>   is a literal in this repository — the change this record says in terms is *not* the one to watch
>   for. No `href` anywhere on the page is derived from a value.
> - **"Any Provider-supplied value becoming displayable without a declared classification behind
>   it."** What this page displays is a Provider's statement of its own terms, not catalogue content
>   about a work: no record, Story, Version or Artwork is drawn on it, and the classification
>   mechanism governs what may be shown *for a record*. **The judgement is recorded rather than
>   assumed**, because a hostile Provider's declaration is prose from an unreviewed service either
>   way, which is why the paragraph above answers it as a prose question and not as a classification
>   one.
>
> **What it adds is no ingress at all.** Nothing a user can reach writes a `source` row: the
> application role holds `SELECT` on that table and no write, and the only thing that can put one
> there is a command an operator types ([`runbook.md`](../runbook.md) → *A Provider's declaration is
> read or re-read*). So the sentence *nobody but the operator can put content here* is untouched, and
> **CAN-113 Add a Provider by pasting its URL** — already on this list — is still the change that
> moves it.
>
> **And what it builds is a control two of these records already lean on.** *Absence is refusal, not
> permission* was a rule stated in the children's assessment and in a ticket; it is now code, with a
> test on each of the two halves this record cares about. That strengthens the position rather than
> changing it, which is why no level moves.
