...
| [transactional-email-providers.md](transactional-email-providers.md) | **Live reference** | Touching email — [CAN-20](https://linear.app/jacobrees-canoncore/issue/CAN-20) or [CAN-31](https://linear.app/jacobrees-canoncore/issue/CAN-31). Resend against Postmark, AWS SES, Mailgun, ZeptoMail, Brevo, MailerSend and SendGrid: free tiers, approval gates, the exact DNS records each demands, terms and sub-processors, and what Google and Yahoo actually require of a low-volume transactional sender. Records what the `canoncore.com` zone already carries, and the steps CAN-20 has to take at Namecheap and Vercel |
