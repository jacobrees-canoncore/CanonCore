...
/**
 * The Story migration 0002 inserts, and the only public one. Named here rather than read from
 * anywhere: this suite drives a deployed URL and has no database connection to ask.
 */
const foundingStory = "Rose";
...
test("one public Story reaches an anonymous visitor", async ({ page }) => {
  await page.goto("/");

  await expect(page.getByRole("listitem")).toHaveText([foundingStory]);
});
