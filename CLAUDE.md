...
- **Migrations are forward-only, and a rollback moves code alone** (ADR-0027) — drizzle-kit ships no down-migration and habit supplies one anyway; it cannot restore what the forward statement destroyed. The rule that replaces it: **every migration must leave the schema able to serve the previous release's code**, so a narrowing waits for a later change.
